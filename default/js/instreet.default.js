...
(function(window,undefined){
	   ...
	   var document = window.document,
		   ...
		   config = {
			    	    // cssurl  :'css/instreet.default.min.css',
        ...
        **********************************/
		...
		var domReady = function (fn){
                ...
                if(document.readyState=="loaded"||document.readyState=="complete") {readylist.push(fn);run();return;}
				if (readylist.push(fn) > 1) return;
				if (document.addEventListener)
				return document.addEventListener('DOMContentLoaded', run, false);
				//IE使用onreadystatechange
				ev.bind(document,'readystatechange',function(){
					if(document.readyState=="loaded"||document.readyState=="complete"){
						ev.remove(document,'readystatechange',arguments.callee);
						run();
					}
				});
		};
		...
		***************************/
		...
		video.prototype={
		    watch:function(){
			    var _=this,container=$('instreet-plugin-container'),wrap=this.container,str,pos=ev.getXY(_.img),w=_.img.width,padTop=Math.round(_.img.height-_.height)/2<0?0:Math.round(_.img.height-_.height)/2,h=_.img.height-padTop;
		...
		**************************************/
		var slider=(function(elem,speed){
			...
			slider.prototype.slideDown=function(callback){
			   var _=this, elem=_.elem,speed=_.speed,
				   start=elem.clientHeight,end=_.height;

			    elem.style.display ="block";
			    _.fx=new Fx(elem,speed,start,end,callback);
				list.push(_.fx);
			};
			...
			var Fx=function(elem,speed,start,end,callback){
		 	...
		 	Fx.prototype.step=function(){
			   var _=this,start=_.start,end=_.end,
			   	   p=(new Date().getTime()-_.startTime)/_.speed,
				   swing=(-(Math.cos(p*Math.PI)/2) + 0.5),
				   gap=end-start;
				if(p<1){
					_.elem.style.height=start+(gap*swing)+'px';
				}else{//动画结束
					_.elem.style.height=end+'px';
					_.callback&&_.callback(); //执行callback
					_.stop();
				}
			};
			//停止动画
		...
		***************************************/
		...
		InstreetAd.prototype={
			...
			locate  :function(){
				var _=this,spots=_.spotHolder.children,icons=_.icons,ad=_.ad,img=_.img,pos=ev.getXY(img),
			...
			bindEvents :function(){
				var _=this,box=_.boxWrapper,icons=_.icons,img=_.img,
					shareIcon=icons.lastChild.children[1],spotHolder=_.spotHolder,ad=_.ad;
				//box onclick
			...
			showWidget    :function(){
				var _=this,spots=_.spotHolder,icons=_.icons;
			...
			showApp         :function(tar){
				var _=this,data=_.data,cn=tar.className,appBox,
